// 銓營場站地址 → 座標。Nominatim 主力 + NLSC 備援，共用既有兩個快取檔。
// 讀 data/parkinsys-raw.json，輸出 data/parkinsys-geo.json（查不到者 lat/lng=null，交給 build 標 geoPending）
...
const raw = JSON.parse(readFileSync(join(ROOT, 'data', 'parkinsys-raw.json'), 'utf8'));
...
for (const lot of raw.lots) {
  ...
  const queries = [addr, road && road !== addr ? lot.city + road : null].filter(Boolean);
  let geo = null;
